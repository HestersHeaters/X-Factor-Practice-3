# =====================================================================
# File: R/Create X Factor Update Graphics.R
# Purpose: Engines for Teams / Hitters / Pitchers; offline-stable:
#          local Quicksand + local logos; identical visuals.
# =====================================================================
...
suppressPackageStartupMessages({
  ...
  library(magick)      # normalize_png, keyline
  library(rlang)
  library(digest)      # baseline hashes
  # base64enc is optional; used for font embedding if present
})
...
normalize_png <- function(in_path,
  ...
  info <- image_info(img)[1, ]; w <- info$width; h <- info$height
...
# Embeds Regular(400)+Bold(700); applies to html/body + all gt elements.
locate_quicksand_local <- function() {
  cands <- c(
    file.path("assets","fonts","quicksand","Quicksand-Regular.woff2"),
    file.path("assets","fonts","quicksand","Quicksand-Bold.woff2"),
    file.path("assets","fonts","quicksand","Quicksand-Regular.ttf"),
    file.path("assets","fonts","quicksand","Quicksand-Bold.ttf"),
    file.path("Quicksand Font","static","Quicksand-Regular.ttf"),
    file.path("Quicksand Font","static","Quicksand-Bold.ttf"),
    file.path("Quicksand Font","Quicksand-VariableFont_wght.ttf")
  )
  ...
  to_mime <- function(p) { ext <- tolower(tools::file_ext(p)); if (ext=="woff2") "font/woff2" else "font/ttf" }
  list(
  ...
  if (!is.null(lf) && has_b64 && (!is.null(lf$regular) || !is.null(lf$bold))) {
    ...
    if (!is.null(reg_uri)) {
      parts <- c(parts, sprintf(
        ...
        reg_uri, if (grepl("woff2", reg_uri)) "woff2" else "truetype"
      ))
    }
    if (!is.null(bld_uri)) {
      parts <- c(parts, sprintf(
        "@font-face{font-family:'QuicksandLocal';src:url('%s') format('%s');font-weight:700;font-style:normal;font-display:swap;}",
        bld_uri, if (grepl("woff2", bld_uri)) "woff2" else "truetype"
      ))
    }
    
    # Apply to entire doc + EVERY gt descendant; keep bold everywhere.
    parts <- c(parts,
...
verify_local_font <- function() {
  ...
  if (!has_b64) {
    message(sprintf("⚠️ Font: local files found (Regular: %s, Bold: %s) but 'base64enc' not installed → falling back to google_font.", reg, bld))
    return(invisible(FALSE))
  }
...
get_logo_map <- function() {
  ...
  abbrs <- c("ARI","ATL","BAL","BOS","CHC","CHW","CIN","CLE","COL","DET","HOU","KCR","LAA",
             "LAD","MIA","MIL","MIN","NYM","NYY","ATH","PHI","PIT","SDP","SEA","SFG","STL",
             "TBR","TEX","TOR","WSN")
  ...
  c(
    ...
    CHC="https://a.espncdn.com/i/teamlogos/mlb/500/chc.png", CHW="https://a.espncdn.com/i/teamlogos/mlb/500/chw.png",
    ...
    HOU="https://a.espncdn.com/i/teamlogos/mlb/500/hou.png", KCR="https://a.espncdn.com/i/teamlogos/mlb/500/kc.png",
    ...
    NYY="https://a.espncdn.com/i/teamlogos/mlb/500/nyy.png", ATH="https://a.espncdn.com/i/teamlogos/mlb/500/oak.png",
    PHI="https://a.espncdn.com/i/teamlogos/mlb/500/phi.png", PIT="https://a.espncdn.com/i/teamlogos/mlb/500/pit.png",
    SDP="https://a.espncdn.com/i/teamlogos/mlb/500/sd.png", SEA="https://a.espncdn.com/i/teamlogos/mlb/500/sea.png",
    SFG="https://a.espncdn.com/i/teamlogos/mlb/500/sf.png", STL="https://a.espncdn.com/i/teamlogos/mlb/500/stl.png",
    TBR="https://a.espncdn.com/i/teamlogos/mlb/500/tb.png", TEX="https://a.espncdn.com/i/teamlogos/mlb/500/tex.png",
    TOR="https://a.espncdn.com/i/teamlogos/mlb/500/tor.png", WSN="https://a.espncdn.com/i/teamlogos/mlb/500/wsh.png"
  )
}

# Helpers to route local vs URL images
...
compute_team_order <- function(filtered_df) {
  stand <- filtered_df %>% dplyr::filter(Team == "Standing") %>% dplyr::select(-Team)
...
build_teams_table <- function(df_input, league = "NL", division = "W") {
  league <- toupper(league); division <- toupper(division)
  df <- df_input
  league_row   <- df[1, -1]
  division_row <- df[2, -1]
  ...
  gt_table <- gt::gt(filtered_df %>% dplyr::mutate(Team = dplyr::recode(
    ...
    gt::tab_style(style = gt::cell_fill(color = "black"), locations = gt::cells_title(groups = "title")) %>%
    gt::tab_style(style = list(gt::cell_text(align = "center", weight = "bold", size = gt::px(20))),
                  locations = gt::cells_body(columns = gt::everything())) %>%
    gt::tab_style(style = gt::cell_text(align = "center", weight = "bold", size = gt::px(20)),
                  locations = gt::cells_column_labels(columns = gt::everything())) %>%
    gt::cols_width(Team ~ gt::px(180), gt::everything() ~ gt::px(120)) %>%
    ...
    gt::tab_style(
      style = list(gt::cell_text(align = "center", color = "black"),
                   gt::cell_borders(sides = "bottom", color = "black", weight = gt::px(10))),
      locations = gt::cells_source_notes()
    ) %>%
    gt::tab_style(style = gt::cell_borders(sides = "bottom", color = "black", weight = gt::px(10)),
                  locations = gt::cells_title(groups = "title")) %>%
    gt::text_transform(locations = gt::cells_body(columns = Team, rows = Team == "Current Playoff %"),
                       fn = function(x) { "Current<br>Playoff %" }) %>%
    gt::fmt_markdown(columns = Team, rows = Team == "Current Playoff %") %>%
    gt::text_transform(locations = gt::cells_body(columns = Team, rows = Team == "Projected Playoff %"),
                       fn = function(x) { "Projected<br>Playoff %" }) %>%
    gt::fmt_markdown(columns = Team, rows = Team == "Projected Playoff %")
  
  for (row_label in c("Wins vs. Projection", "Playoff % vs. Projection")) {
    for (team_col in team_order) {
      gt_table <- gt_table %>%
        gt::tab_style(style = gt::cell_fill(color = "#ff9999"),
                      locations = gt::cells_body(columns = team_col,
                                                 rows = Team == row_label & as.numeric(gsub("[^0-9.-]", "", .data[[team_col]])) > 0)) %>%
        gt::tab_style(style = gt::cell_fill(color = "#99ccff"),
                      locations = gt::cells_body(columns = team_col,
                                                 rows = Team == row_label & as.numeric(gsub("[^0-9.-]", "", .data[[team_col]])) < 0))
    }
  }
  gt_table <- gt_table %>%
    gt::tab_style(style = gt::cell_borders(color = "lightgray", sides = c("top", "bottom", "left", "right")),
                  locations = gt::cells_body()) %>%
    gt::tab_style(style = gt::cell_borders(sides = "bottom", color = "black", weight = gt::px(5)),
                  locations = gt::cells_column_labels(columns = gt::everything())) %>%
    gt::tab_style(style = gt::cell_text(size = gt::px(20), weight = "bold"),
                  locations = gt::cells_column_labels(columns = gt::everything()))
  gt_table
}
...
apply_cell_fill <- function(gt_table, column, colors) {
  for (i in seq_along(colors)) {
    gt_table <- gt_table %>%
      gt::tab_style(style = gt::cell_fill(color = colors[i]),
...
build_hitters_table <- function(df_input, league = "AL", division = "W") {
  ...
  formatted <- wide %>%
    ...
    dplyr::mutate(
      ...
      AVG_color = get_color(AVG_diff), OBP_color = get_color(OBP_diff), SLG_color = get_color(SLG_diff),
      wOBA_color = get_color(wOBA_diff), K_color = get_color(K_diff), BB_color = get_color(BB_diff),
      wRC_color = get_color(wRC_diff), WAR_color = get_color(WAR_adj_diff)
    )
  ...
  gt_table <- formatted %>%
    ...
    embed_quicksand_or_fallback() %>%
    gt::tab_style(style = list(gt::cell_text(color = "white", weight = "bold"), gt::cell_fill(color = "black")),
                  locations = gt::cells_title(groups = c("title", "subtitle"))) %>%
    gt::tab_style(style = list(gt::cell_text(color = "white", weight = "bold"), gt::cell_fill(color = "black")),
                  locations = gt::cells_column_labels(columns = gt::everything())) %>%
    gt::tab_style(style = gt::cell_text(size = gt::px(18)), locations = gt::cells_column_labels()) %>%
    gt::tab_style(style = gt::cell_text(size = gt::px(18)), locations = gt::cells_body()) %>%
    gt::tab_options(heading.align = "center", table.align = "center",
                    data_row.padding = gt::px(12), table_body.hlines.color = "lightgray") %>%
    gt::tab_style(style = gt::cell_borders(sides = "bottom", color = "red", weight = gt::px(5)),
                  locations = gt::cells_title(groups = "subtitle")) %>%
    gt::tab_style(style = gt::cell_borders(sides = "all", color = "lightgray", weight = gt::px(1)),
                  locations = gt::cells_body(columns = gt::everything())) %>%
    gt::tab_style(style = gt::cell_borders(sides = "right", color = "transparent", weight = gt::px(1)),
                  locations = gt::cells_body(columns = Team)) %>%
    gt::fmt_number(columns = c(AVG_Current, OBP_Current, SLG_Current, wOBA_Current), decimals = 3) %>%
    gt::fmt_number(columns = c(`K%_Current`, `BB%_Current`, WAR_Current), decimals = 1) %>%
    gt::fmt_number(columns = `wRC+_Current`, decimals = 0)
...
build_pitchers_table <- function(df_input, league = "AL", division = "E") {
  ...
  gt_table <- Formatted_Table %>%
    dplyr::select(Team, Player, ERA_Current, FIP_Current, `K%_Current`, `BB%_Current`, WHIP_Current, WAR_Current) %>%
    gt::gt() %>%
    gt::text_transform(
      locations = gt::cells_body(columns = Team),
      fn = function(x) {
    ...
    gt::cols_label(ERA_Current = "ERA", FIP_Current = "FIP",
    ...
    gt::tab_style(style = gt::cell_borders(sides = "right", color = "transparent", weight = gt::px(1)),
                  locations = gt::cells_body(columns = Team)) %>%
    gt::fmt_number(columns = c(ERA_Current, FIP_Current, WHIP_Current), decimals = 2) %>%
    gt::fmt_number(columns = c(`K%_Current`, `BB%_Current`, WAR_Current), decimals = 1)
...
run_with_excel <- function(input_xlsx,
  ...
  ensure_parent_dir(output_png); ensure_parent_dir(output_html)
