...
# Baseline file path per kind/league/division (sheet-agnostic)
baseline_path_for <- function(league, division, kind) {
  k <- tolower(kind)
  if (k == "standings") k <- "teams"
  file.path("build", sprintf("baseline_%s_%s_%s.txt", k, toupper(league), toupper(division)))
}

# One-time migration (old -> new baseline names)
migrate_standings_to_teams_baselines <- function(build_dir = "build") {
  if (!dir.exists(build_dir)) return(invisible(FALSE))
  olds <- list.files(build_dir, pattern = "^baseline_standings_[A-Z]{2}_[WEC]\\.txt$", full.names = TRUE)
  if (!length(olds)) return(invisible(FALSE))
  for (old in olds) {
    new <- sub("baseline_standings_", "baseline_teams_", old)
...
migrate_standings_to_teams_baselines("build")
